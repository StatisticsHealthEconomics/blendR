...
obs_surv <- shiny::reactive({
    ...
    obs_inla <- surv_est_inla(inla.formula = as.formula(paste("inla.surv(", input$time, ",", input$event, ") ~ -1",sep = "")),
                                      data = dat(),
    ...
    S_obs <- make_surv(obs_inla(), t = seq(input$min, input$max, by = surv_step), nsim = 100)
  ...
  ext_surv <- shiny::eventReactive(input$plot,{
   ...
   data_sim <- ext_surv_sim(t_info = tp_ext,
    ...
    ext_mod <- fit.models(formula = Surv(time, event)~1,
                             data = data_sim,
                            distr = "gompertz",
                           method = "hmc",
  ...
  output$extplot <- shiny::renderPlot({
    ...
    ggplot() +
      ...
      geom_line(aes(time, rowMeans(ext_surv()$S_ext)), color = "#00BFC4", size = 1.25)+
  ...
  ble_surv <- shiny::reactive({
    ...
    blend_interv <- list(min = input$a, max = input$b)
    beta_params  <- list(alpha = input$alpha, beta = input$beta)

    blendsurv(obs_surv()$mod, ext_surv()$mod, blend_interv(), beta_params())
