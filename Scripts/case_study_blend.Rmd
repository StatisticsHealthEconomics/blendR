...
- `prog` : Progression status 0 = censored, 1 = progression
- `death`: Death status 0 = censored, 1 = dead
- `prog_t`: Progression time in months
- `death_t`: Survival time in months
- `prog_ty`: Progression time in years
- `death_ty`: Survival time in years

Before starting the analysis, we need to source a R script that contains all functions required in the following components firstly.   
```{r source function, include=TRUE}
source("../Scripts/Functions.R", local = knitr::knit_global())
```
Details of the functions are described in another chapter *Useful functions*. 
...
```{r observedplot, echo=FALSE, fig.cap="Survival estimates of the FCR arm based on the piecewise exponential model fitted to the observed data"}
...
```{r blendedplot, echo=FALSE, fig.cap="Blended survival curve based on short-term data and external information for FCR arm"}
ggplot() + geom_line(aes(obs_Surv$KM$time, obs_Surv$KM$surv, colour = "Kaplan-Meier"), size = 1.25, linetype = "dashed")+
  xlim(0, 180) + ylim(0,1) +
