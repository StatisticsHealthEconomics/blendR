...
surv_est_inla <- function(time, event, data,
  ...
  surv.formula <- as.formula(paste("Surv(", time, ",", event, ")~1", sep = ""))

  km <- survfit(surv.formula, data = data)
...
# ggplot for survival estimate

obsurvplot <- function(time, S_obs, KM_time, KM_surv){

  ggplot() +
    geom_line(aes(KM_time, KM_surv, colour = "Kaplan-Meier"), size = 1.25, linetype = "dashed") +
    xlim(0, max(time)) + ylim(0,1) +
    geom_line(aes(time,rowMeans(S_obs), colour = "Data fitting"), size = 1)+
    geom_ribbon(aes(x=time, y = rowMeans(S_obs),
                    ymin = apply(S_obs, 1, quantile, probs = 0.025),ymax = apply(S_obs, 1, quantile, probs = 0.975)), alpha = 0.1)+
...
#'
extsurvplot <- function(time, S_ext){

  ggplot() +
    xlim(0, max(time)) + ylim(0,1) +
    geom_line(aes(time, rowMeans(S_ext)), color = "#00BFC4", size = 1.25)+
    geom_ribbon(aes(x=time, y = rowMeans(S_ext), ymin = apply(S_ext, 1, quantile, probs = 0.025),
                    ymax = apply(S_ext, 1, quantile, probs = 0.975)), alpha = 0.1) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
          axis.line = element_line(colour = "black"), text = element_text(size = 12))+
    xlab("Time (months)") + ylab("Survival")
...
#'
blesurvplot <- function(obs_Surv, ext_Surv, ble_Surv, tp){
